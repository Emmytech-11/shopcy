...
const itemStyle = `
        relative before:content-[""] before:w-[80%] before:h-1 
          before:absolute before:bottom-[-4px] before:left-0 before:scale-0 before:transition-all
          before:duration-500 before:origin-left hover:before:scale-100 
      ...
      ${showMenu ? "left-0" : "left-[-100%]"}`}>
        <ul className='mt-24 md:mt-0 px-5 md:px-0 flex flex-col md:flex-row md:items-center
         gap-5 font-bold text-xl md:text-base text-black md:text-white'>
          <li className={itemStyle} onClick={() => setShowMenu(false)} >
            <Link href="/">Home</Link>
          </li>
          <li className={itemStyle} onClick={() => setShowMenu(false)} >
